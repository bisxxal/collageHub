        ...
        <div className="flex flex-col gap-2 mt-6 pl-3 " key={i.title} >
          {i.items.map((item) => {
            if (item.visible.includes(role)) {
              return (
                <Link href={item.href} key={item.label}   className={` ${trimmedPathname == (item.label.toLowerCase()) ? '  !text-[#3352cc] !font-semibold  ': '  '} flex text-xl w-full pl-3 items-center hover:translate-x-4  transition-all justify-start lg:justify-start gap-4 text-gray-500 py-2 md:px-2 rounded-md `}>
                  {item.icon}
